...
term that stiffens the P-loop region upon ligand binding, which allows us to switch
individual subunits between apo-like and bound-like states without re-parameterizing
the entire network. Normal mode analysis of the resulting structures shows that the
three lowest non-trivial modes reorganize substantially when two adjacent subunits are
occupied, whereas occupancy of two subunits on opposite sides of the ring leaves the
low-frequency spectrum nearly unchanged, indicating that the allosteric channel is
strongly directional. We quantify this directionality with a perturbation-response
susceptibility computed from the network covariance matrix, and we find that the
susceptibility decays with an effective length constant of 2.3 subunits in the
counter-clockwise direction and 0.7 subunits in the clockwise direction. Brownian
dynamics simulations of the same network under a periodic chemical driving protocol
reproduce the sequential firing order observed in single-molecule rotation assays [12],
and the mean rotation rate saturates at high driving frequency in a manner consistent
with a rate-limiting mechanical relaxation step rather than a rate-limiting chemical
step. We further demonstrate for the first time that truncating the C-terminal helix of
a single subunit is sufficient to abolish the directionality of the susceptibility
without measurably changing the equilibrium fluctuation amplitude of the ring, which
suggests that the directional channel is carried by a small number of specific contacts
rather than by the global elastic architecture. These results provide a minimal
physical picture of intersubunit communication in ring ATPases and a computationally
inexpensive route to predicting which contacts dominate allosteric coupling in other
oligomeric molecular machines.
...
Ring-shaped ATPases are among the most important molecular machines in the cell, and
understanding how their subunits coordinate is a long-standing problem in biophysics.
This study introduces a coarse-grained elastic network model in which the nucleotide
state of each subunit can be switched independently, making it possible to map the
directionality of allosteric communication around the ring at very low computational
cost. We show that the communication channel is strongly asymmetric and that it is
carried by a small number of specific contacts near the C-terminal helix, a prediction
that can be tested directly by truncation experiments. The approach is general and
should be applicable to a wide range of oligomeric machines, including chaperonins,
helicases, and proteasomal unfoldases, where similar coordination problems arise.
...
observed in single-molecule assays. A preliminary version of this work was deposited
...
establishing which conformations are accessible (PDB 6qw9). What the static picture
...
Overdamped Langevin dynamics were integrated with a time step of 20 fs and a uniform
...
run covered 200 ns and thirty independent runs were performed for each condition.
...
rabbit skeletal muscle by the standard procedure. Rotation of a 200 nm bead attached to
...
counter-clockwise direction and 0.7 subunits in the clockwise direction (Figure 4).
...
the channel becomes effectively isotropic. The equilibrium root-mean-square
...
K.M., A.T., and R.N. declare that they have no conflict of interest.
...
[8] Kuramoto, H. Personal communication, 2025.
...
Tables
  ...
  ------------------------------------------------------------------
  Footnotes: 1. Cutoff applies to non-local springs only. 2. Truncation removes
  residues 471-489 of subunit A. 3. Counts exclude nucleotide beads.
...
Figure 3. Residue-level decomposition of the susceptibility change upon C-terminal
helix truncation, shown as a heat map over the subunit interface.

Figure 4. Perturbation-response susceptibility as a function of subunit separation in
...
Novel coarse-grained model of allosteric communication in a ring-shaped ATPase. A
coarse-grained elastic network representation of a hexameric ring ATPase is used to
map the directionality of allosteric communication between subunits. Each subunit can
be independently switched between an apo-like and a nucleotide-bound-like state by a
stiffening term applied to the P-loop region, and the resulting perturbation-response
susceptibility is computed analytically from the network covariance matrix. The
susceptibility decays much more slowly in the counter-clockwise direction than in the
clockwise direction, revealing a strongly directional allosteric channel that is
carried by a small number of contacts involving the C-terminal helix. Truncation of
that helix abolishes the directionality while leaving the global fluctuation amplitude
of the ring essentially unchanged, and the same analysis applied to other oligomeric
machines is expected to identify comparable contact sets.
