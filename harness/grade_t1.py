...
# The rule actually applied in match() below: an item counts as detected when a
# finding contains one of its anchors and either that anchor is at least
# ANCHOR_ALONE characters long or the finding also contains at least SUPPORT of the
# item's supporting keywords. Two earlier constants (MIN_HITS = 2, DISTINCTIVE = 10)
# described a stricter rule than the code has ever applied and are removed rather
# than left to contradict it; changing the rule itself would change every score and
...
def match(gt_items, findings):
    ...
    for gi, v in enumerate(gt_items):
        ...
        for fi, txt in enumerate(texts):
            ...
            if max(len(k) for k in a) >= ANCHOR_ALONE or len(sup) >= SUPPORT:
                score = len(a) + len(sup)
        ...
        for fi, txt in enumerate(texts):
            if any(k not in txt for k in need):
                continue
            a = [k for k in anchors if k.lower() in txt]
            if a and (max(len(k) for k in a) >= ANCHOR_ALONE
                      or len(hits(v['match_keywords'], txt)) >= SUPPORT):
                used_f.add(fi)
    return pairs, used_f
